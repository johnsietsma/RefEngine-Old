...
std::shared_ptr<Material> MaterialManager::LoadMaterial(AssetManager& assetManager, const MaterialDefinition& materialDefinition)
{
    ...
    ProgramId programId = assetManager.LoadProgram(materialDefinition.vertexShaderName, materialDefinition.fragmentShaderName);
    auto material = std::make_shared<Material>(programId);

    if (materialDefinition.textureName!=nullptr)
    {
        const Texture& texture = assetManager.LoadTexture(materialDefinition.textureName);
